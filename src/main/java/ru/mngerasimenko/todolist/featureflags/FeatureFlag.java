...
public enum FeatureFlag {
    ...
    TODO_REMINDER_EMAIL("app.todo-reminder-email.enabled", false, Audience.SERVER, OverrideLifetime.PERSISTENT,
            "Канал ПИСЕМ в напоминаниях о сроках. Отделён от TODO_REMINDERS, потому что push и " +
            "почта готовы к запуску по-разному: push ничем не заблокирован, а письма ждут двух " +
            "решений owner'а — токен отписки общий на все типы писем, и согласие на письма о " +
            "сроках сейчас можно только выключить, включить обратно нечем. Флаг позволяет " +
            "запустить напоминания push'ами, не трогая согласия живых пользователей и не " +
            "дожидаясь этих решений. Проверяется ДО выпуска токена отписки: тот пишет в общую " +
            "с маркетинговой рассылкой колонку, и выпускать его вхолостую нельзя."),
    ...
    TODO_UPDATE_SYNC_PUSH("app.todo.update-sync-push.enabled", false,
            "Тихий data-push участникам списка при ИЗМЕНЕНИИ задачи. Уведомления не показывает: " +
            "в сообщении нет notification-payload, только data, поэтому клиент обновляет список " +
            "и ничего не рисует. Закрывает ПРАВКУ через PUT из дыры, найденной на проде " +
            "08.09.2026; снятие галочки (PATCH .../undone) и удаление задачи идут отдельными " +
            "эндпоинтами и по-прежнему НЕ рассылаются — там тот же класс бага остаётся. Сервер шлёт " +
            "push на создание задачи и на её выполнение, а на правку — ничего, и у участника " +
            "оставалась строка в состоянии «до правки». Получают ВСЕ участники, включая второе " +
            "устройство самого редактора: сообщение невидимое, исключать некого. " +
            "ЧТО ЗНАТЬ ПЕРЕД ВКЛЮЧЕНИЕМ. (1) При закрытом приложении проверено на устройстве " +
            "10.09.2026 (staging, Honor REA-NX9 / Android 15): процесс поднимается, список " +
            "обновляется за ~1,5 секунды, в шторке ничего не появляется. В принудительном Doze " +
            "процесс не поднялся за 50 секунд, но сразу после выхода из Doze обновление прошло — " +
            "сообщение откладывается, а не теряется. Прежняя оговорка «эффект не гарантирован» " +
            "(fire-and-forget после отпускания wakelock, EMUI режет data-only) замером не " +
            "подтвердилась. Одно устройство, один прогон, приложение было недавно активным " +
            "(лучший bucket App Standby) — у редко используемого приложения задержка в Doze " +
            "будет дольше. " +
            "(2) Нагрузка: сообщение на каждую правку каждому ТОКЕНУ (у участника с двумя " +
            "устройствами — два), и каждый получатель в ответ тянет ВЕСЬ список. Ограничителя " +
            "частоты нет, схлопывание спасает только очередь офлайн-устройства. " +
            "(3) Выключение останавливает только НОВЫЕ отправки: уже принятые FCM сообщения живут " +
            "до часа (TTL), то есть хвост после выключения — до 60 минут. " +
            "(4) Переключение PERSISTENT: переживает рестарт и деплой, вернуть можно только " +
            "через DELETE. " +
            "Работает и на уже опубликованных сборках: обновление списка по любому push'у с " +
            "list_id есть начиная с 1.1.7 (vc13; коммит aad1f1a от 11.04.2026, вышел в бампе " +
            "10f6c6e). Выключен по умолчанию — рассылка, включать после проверки на staging."),

    CLIENT_TODOLIST_KEEP_TOP("client.todolist.keep-top.enabled", true, Audience.CLIENT,
            "«Остаёмся наверху» в списке задач Android (1.2.7): если список стоял в самом верху, а над " +
            "верхней строкой появилась новая (задачу добавил соавтор, пришла с сервера после " +
            "отрисовки кэша), список остаётся наверху и новая задача видна. Без этого LazyColumn " +
            "держит позицию по прежней верхней строке, и новая задача оказывается выше экрана — баг " +
            "с проверки 1.2.7 на двух телефонах 11.09.2026. Прокрученный пользователем список правило " +
            "не трогает. Побочный эффект: если бывшая верхняя строка уехала вниз (пересортировка, " +
            "переименование при алфавитной сортировке), список больше не едет за ней, а остаётся " +
            "наверху. При false прокрутка ведёт себя в точности как до 1.2.7. Выключать, если этот " +
            "побочный эффект окажется хуже исходного бага. Действует со следующего запуска приложения."),
