...
public enum FeatureFlag {
    ...
    TODO_UPDATE_SYNC_PUSH("app.todo.update-sync-push.enabled", false,
            ...
            "ЧТО ЗНАТЬ ПЕРЕД ВКЛЮЧЕНИЕМ. (1) Эффект не гарантирован при закрытом приложении: " +
            "клиент запускает обновление незавершаемой корутиной уже после возврата из " +
            "onMessageReceived, а сервис к этому моменту отпускает wakelock; плюс EMUI/MagicOS " +
            "режут фоновую доставку data-only. На устройстве при закрытом приложении НЕ проверено " +
            "— это обязательное условие перед включением на проде. В foreground работает. " +
