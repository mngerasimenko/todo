...
public interface PushNotificationService {
     ...
     * на такой push перечитывает список — этот путь есть в Android-клиенте начиная с 1.1.7
     * (vc13), поэтому отдельной клиентской правки не требуется, чтобы ничего не сломалось.
     * <p>
     * Замерено на устройстве 10.09.2026 (staging, Honor REA-NX9 / Android 15): при УБИТОМ
     * приложении и бодрствующем телефоне процесс поднимается и список обновляется за ~1,5 с,
     * в шторке ничего не появляется; в принудительном Doze процесс не поднимается за 50 с, но
     * сразу после выхода из Doze обновление проходит — сообщение откладывается, а не теряется.
     * Прежняя запись «эффект не гарантирован» (незавершаемая корутина после того, как сервис
     * отпустил wakelock) была догадкой по коду и замером не подтвердилась; в пределе гарантию
     * всё равно дала бы клиентская правка (WorkManager вместо fire-and-forget). Оговорка: одно
     * устройство, один прогон, приложение было недавно активным (лучший bucket App Standby).
     * <p>
