...
# между проверками — это пауза переподключения плюс два таймаута curl
# (15 с у вызова метода, 35 с у опроса). Отсюда худшая жизнь процесса:
# LP_DEAD_AFTER + (LP_RETRY_SLEEP + 50) + RestartSec. При потолках ниже это
# 150 + 80 + 10 = 240 с, пять таких попыток — 1200 с, окно в юните 1500 с.
