...
# и дальше пороги пусты. Каждое сравнение тогда падает в лог с «integer
# expression expected», ни один алерт не срабатывает, а прогон выходит с нулём —
# мониторинг выключен и молчит об этом. Поэтому проверяем не читаемость файла, а
# то, без чего прогон бессмыслен; установщик рядом поступает так же (он тоже
# смотрит не файл, а наличие VK_TOKEN внутри).
if [ -z "${VK_TOKEN:-}" ] || [ -z "${RAM_WARN:-}" ] || [ -z "${SWAP_WARN:-}" ] || [ -z "${DISK_WARN:-}" ]; then
    echo "server-monitor: в ${SCRIPT_DIR}/monitor.conf нет учётки VK или порогов — алерты не сработали бы ни разу" >&2
    exit 1
fi
