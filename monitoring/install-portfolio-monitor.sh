...
if [ -n "$CONF_ROLE" ] && [ "$CONF_ROLE" != "$ROLE" ]; then
  die "хост установлен как ${CONF_ROLE}, а запущено ${ROLE} — так снялись бы чужие строки crontab"
fi
...
tighten_conf() {
  ...
  case "$after" in
    ...
    *)       echo "ВНИМАНИЕ: не сузил права ${f} (сейчас ${after:-неизвестно}) — секрет читают лишние" >&2 ;;
  esac
}
