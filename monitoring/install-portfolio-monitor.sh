...
case "$CONF_ROLE" in
  ...
  # ровно так же, как о нём говорит сама опрашивалка. Но при взведённом
  # выключателе не роняем установку: правило всего файла — снять строку
  # сломанной опрашивалки важнее придирок к её конфигу, иначе выключить её было
  # бы нельзя, пока конфиг не починишь.
  *)
    if [ "$DISABLED" = "0" ]; then
      die "в ${CONF} испорчена роль (ROLE=${CONF_ROLE}): ожидалось external или peer"
    fi
    echo "ВНИМАНИЕ: в ${CONF} испорчена роль (ROLE=${CONF_ROLE}) — при выключателе это не мешает снять строку" >&2
    CONF_ROLE=""
    ;;
esac
...
# Сверка по конфигу выше молчит там, где конфига ещё нет, он без ROLE или не
# сорсится. Вторая половина той же защиты — по факту: помеченная строка, которую
# ЭТА роль не ведёт, значит хост ставили другой ролью. Смотрим только на метку,
# без filter_crontab: тот зависит от CRON_*, которых здесь ещё нет, а нам нужно
...
# Путь к конфигу с токеном VK — из конфига хоста, тем же приёмом с дескриптором,
# что и роль выше: `trap ... EXIT` в чужом файле иначе приклеился бы к значению.
