...
# Роль хоста записана в его же конфиг при первой установке. Сверяем её ДО всего
# остального: роль external не ставит ни строку продления сертификатов, ни прогон
# vpscan, но метку снимает со ВСЕХ помеченных строк — значит один неверный
# аргумент на проде уносил две чужие задачи и отчитывался об успехе. Обе команды
# стоят рядом в README, перепутать их дёшево.
#
# Стоит это здесь, а не ниже, потому что отказ не должен оставлять следов: ниже
# уже раскладываются root-копии скриптов, logrotate и отметка продления, и после
# отказа их пришлось бы убирать руками, не зная, что они появились.
#
# stdout подоболочки глушим целиком через exec, а ответ ведём дескриптором 3:
# конфиг — чужой файл, и `trap ... EXIT` в нём печатает уже ПОСЛЕ нашей
# подстановки, когда перенаправление самой команды `.` не действует, — его
# вывод приклеился бы к прочитанному значению.
CONF_ROLE="$( unset ROLE; exec 3>&1 >/dev/null 2>&1; . "$CONF"; printf '%s' "${ROLE:-}" >&3 )"
case "$CONF_ROLE" in
  ''|external|peer) ;;
  # Испорченное значение — это сломанный конфиг, а не ошибка в команде; говорим
  # ровно так же, как о нём говорит сама опрашивалка.
  *) die "в ${CONF} испорчена роль (ROLE=${CONF_ROLE}): ожидалось external или peer" ;;
esac
if [ -n "$CONF_ROLE" ] && [ "$CONF_ROLE" != "$ROLE" ]; then
  die "хост установлен как ${CONF_ROLE}, а запущено ${ROLE} — так снялись бы чужие строки crontab. Если роль хоста действительно меняется, поправь ROLE= в ${CONF} и сними лишние строки сам"
fi
...
for suite in $SUITES; do
  ...
  # env -u: фильтр из окружения превратил бы гейт в прогон подмножества, и
  # самопроверка набора «объявлено/отчиталось» при этом отключается.
  env -u PM_TEST_FILTER -u VK_TEST_FILTER bash "${SRC}/tests/${suite}.test.sh" || die "тесты ${suite} не прошли"
done
...
# Роль хоста записана в его же конфиг при первой установке. Сверяем: роль
# external не ставит ни строку продления сертификатов, ни прогон vpscan, но
# метку снимает со ВСЕХ помеченных строк — значит один неверный аргумент на
# проде уносил две чужие задачи и отчитывался об успехе. Обе команды стоят рядом
# в README, перепутать их дёшево. Конфига без роли (поставлен руками до этой
# проверки) хватает как есть — там сверять нечего.
VK_CONF="$( unset STATE_DIR VK_CONF_FILE; exec 3>&1 >/dev/null 2>&1; . "$CONF"; printf '%s' "${VK_CONF_FILE:-}" >&3 )"
...
tighten_conf() {
  local f="$1" mode after
  # Симлинк проверяем ПЕРВЫМ: -e идёт по ссылке, и битая ссылка на месте конфига
  # уходила бы отсюда молча, без обещанного предупреждения.
  if [ -L "$f" ]; then
    echo "ВНИМАНИЕ: ${f} — симлинк, права не трогаю; проверь руками, там токен VK" >&2
    return 0
  fi
  [ -e "$f" ] || return 0
  # Каталог проходит -e и не ловится -L. chmod 600 снял бы с него бит обхода для
  # всех, кроме root, то есть сломал бы чужой каталог, если VK_CONF_FILE в
  # конфиге хоста указывает не на файл.
  if [ ! -f "$f" ]; then
    echo "ВНИМАНИЕ: ${f} — не обычный файл, права не трогаю; проверь руками, там токен VK" >&2
    return 0
  fi
  ...
  # Судим по перечитанному режиму, а не по коду возврата: ACL, монтирование с
  # fmask и noperm-раздел возвращают ноль, ничего не изменив, — и установщик
  # рапортовал бы «права сужены» про секрет, который по-прежнему читают все.
  # Для crontab он записанное сверяет честно; здесь должно быть так же.
  # Между проверкой и chmod файл могли подменить симлинком: предотвратить это в
  # sh нечем (chmod без O_NOFOLLOW), а заметить — можно, и молчать об этом
  # нельзя: каталог второй копии принадлежит deploy.
  if [ -L "$f" ] || [ ! -f "$f" ]; then
    echo "ВНИМАНИЕ: ${f} подменили во время установки — сейчас это не обычный файл; проверь руками, там токен VK" >&2
    return 0
  fi
  after="$(stat -c '%a' -- "$f" 2>/dev/null)"
  case "$after" in
    600|400) echo "--- права сужены до 600: ${f} ---" ;;
    *)       echo "ВНИМАНИЕ: не сузил права ${f} (сейчас ${after:-неизвестно}) — секрет читают лишние. Вероятные причины: ACL, монтирование с fmask, ФС без прав; проверь: stat -c '%a %U:%G' ${f}" >&2 ;;
  esac
}
...
# Без фильтра по -r: он идёт по ссылке, и битая ссылка на месте конфига сюда бы
# не дошла — а предупредить о ней умеет как раз tighten_conf.
tighten_conf "$VK_CONF"
...
# Сверка роли по конфигу выше молчит там, где конфига ещё нет, он без ROLE или не
# сорсится (взведён выключатель). Поэтому вторая половина той же защиты — по
# факту: помеченная строка, которую ЭТА роль не ведёт, значит хост ставили другой
# ролью, и продолжать нельзя — её сняли бы и не вернули.
if [ "$ROLE" = "external" ]; then
  PEER_MANAGED="$(filter_crontab drop | grep -E 'certbot-renew\.sh|run-local\.ts' | head -1)"
  [ -z "$PEER_MANAGED" ] || die "в crontab есть помеченная строка роли peer (${PEER_MANAGED}) — хост ставили как peer, а запущено external: эти строки снялись бы и не вернулись"
fi
