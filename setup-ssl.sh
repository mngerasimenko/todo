...
echo ""
echo "Автообновление сертификатов и перезагрузка nginx:"
echo "  bash monitoring/install-portfolio-monitor.sh peer   # из-под root"
echo ""
echo "Он ставит monitoring/certbot-renew.sh почасовым cron'ом. Строку"
echo "  0 */12 * * * docker exec certbot certbot renew --quiet && ... nginx -s reload"
echo "вручную больше не добавлять: установщик её снимает, а reload теперь висит"
echo "на --deploy-hook самого certbot (см. monitoring/README.md)."
