...
<a href="https://github.com/devxb/gitanimals">
  ...
  <img src="https://render.gitanimals.org/lines/mngerasimenko?pet-id=846821718390091682" width="30%" height="100"/>
  <img src="https://render.gitanimals.org/lines/mngerasimenko?pet-id=843814633071056111" width="30%" height="100"/>  
</a>
...
- Runtime feature-flags через `/api/admin/flags/{name}/{value}` (super-admin). Приоритет: runtime → env → enum-default. Переключение флагов **фич** сохраняется в БД и переживает рестарт и деплой; флаги **защиты** (`rate-limit`, `response-cache`) восстанавливаются сами после рестарта.
- Клиентские feature-flags: `GET /api/status` отдаёт `client_flags` — только флаги с `Audience.CLIENT`, серверные наружу не уходят. Android забирает их при старте и кэширует, переключаются тем же `PUT /api/admin/flags/{name}/{value}`. Это выключатель для функций, которые исполняет само приложение: публикация в RuStore ручная и небыстрая, без него сломанную клиентскую функцию не погасить неделями
...
- **Resilience: Redis circuit breaker** — при недоступности Redis приложение работает с latency ~30 мс на запрос (а не 60 сек / HTTP 500). Двухуровневая защита: (1) reactive fallback на in-memory для cache / rate-limit / blacklist; (2) proactive circuit breaker через `RedisHealthService` — после первой ошибки запросы пропускают Redis полностью до восстановления (PING каждые 30 сек). `/api/status` отдаёт `redis_healthy: bool`, VK-бот шлёт алерт раз в сутки. Метрики `cache_errors_total{cache,operation}` + `redis_fallback_total{component}` в Grafana dashboard «Redis Cache (Spring)»
- Unit-тесты с проверкой покрытия (JaCoCo) + интеграционные на TestContainers (concurrency, Redis blacklist, Redis rate-limit, Redis user-auth cache, долговечность feature-флагов) — отдельный запуск через `-Pintegration`
